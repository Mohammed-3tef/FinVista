...
function buildAnalyticsHtml(data: AnalyticsReportData): string {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8">${BASE_STYLES}</head><body>
    <div class="page">
      <div class="header">
        <div class="brand">
            ...
            <div class="brand-info">
                <p class="brand-name">Fin<span>Vista</span></p>
      ...
      <div class="footer">Generated by FinVista · ${data.generatedAt}</div>
    </div>
  </body></html>`;
}
...
export async function exportAnalyticsPDF(data: AnalyticsReportData): Promise<void> {
  ...
  const result = await generatePDF({
    html,
    fileName: `FinVista_Analytics_${Date.now()}`,
  });
  ...
  await Share.open({
    ...
    title: 'FinVista Analytics Report',
    failOnCancel: false,
  });
}
...
export async function exportGoalProgressPDF(data: GoalProgressReportData): Promise<void> {
  ...
  const result = await generatePDF({
    html,
    fileName: `FinVista_Goal_${safeName}_${Date.now()}`,
  });
